...
enum reg SetupBinaryInvariant(DAST* dast, char* startLabel, char* endLabel, int regBytes) {
  ...
  Dispatch(child1, startLabel, endLabel, regBytes);
  EmitADDI(SP, SP, -4); //Put address in
  EmitSW(S1, 0, SP);
...
void ProcessExprBinaryAdd(DAST* dast,
  ...
  if ((c1PtrTotal > 0) && (c2PtrTotal > 0)) {
		printf("Error. both add are pointers.");
		return;
	} else if (c1PtrTotal > 0) {
		if (c1PtrTotal > 1) {
			EmitSLLI(T1,S1,2);
			EmitADD(S1,firstReg,T1);
		} else {
			EmitADDI(T1,X0,child1->typeDecl->dataSize);
			EmitMUL(T2,T1,S1);
			EmitADD(S1,firstReg,T2);
		}
	} else if (c2PtrTotal > 0) {
		if (c2PtrTotal > 1) {
			EmitSLLI(T1,firstReg,2);
			EmitADD(S1,S1,T1);
		} else {
			EmitADDI(T1,X0,child2->typeDecl->dataSize);
			EmitMUL(T2,T1,firstReg);
			EmitADD(S1,S1,T2);
		}
	} else {
...
void ProcessExprBinarySub(DAST* dast,
  ...
  /* STUDENT CODE HERE. */
  if ((c1PtrTotal > 0) && (c2PtrTotal > 0)) {
		int first = child1->typeDecl->dataSize;
		int second = child2->typeDecl->dataSize;
		if (c1PtrTotal > 1) {
			first = 4;
		}
		if (c2PtrTotal > 1) {
		...
		if (first != second) {
			printf("Different sizes detected in both sub operands\n");
			return;
		}
		EmitSUB(T1,firstReg,S1);
		EmitADDI(T2,X0,first);
		EmitDIV(S1,T1,T2);
	} else if (c1PtrTotal > 0) {
		if (c1PtrTotal > 1) {
			EmitSLLI(T1,S1,2);
...
void ProcessExprBinaryGTEq(DAST* dast,
  ...
  /* STUDENT CODE HERE. */
  if ((c1PtrTotal > 0) && (c2PtrTotal > 0)) {
		if ((c1PtrTotal != c2PtrTotal) || (child1->typeDecl != child2->typeDecl)) {
			printf("Pointers being compared do not have the same type. \n");
			return;
		}
		EmitSLTU(T1,firstReg,S1);
		EmitSLTIU(S1,T1,1);
	} else if (c1PtrTotal > 0) {
		printf("Can't compare GTE. Invalid input. \n");
		return;
	} else if (c2PtrTotal > 0) {
		printf("Can't compare GTE. Invalid input. \n");
		return;
		...
		int first, second;
		if (child1->typeDecl == uintType) {
			first = 4;
		} else if (child1->typeDecl == intType) {
			first = 3;
		} else if (child1->typeDecl == ucharType) {
			first = 2;
		} else if (child1->typeDecl == charType) {
			first = 1;
		} else if (child1->typeDecl == nullType) {
			first = 0;
		} else {
			printf("First argument doesn't correspond to any valid int type. \n");
		}
		if (child2->typeDecl == uintType) {
			second = 4;
		} else if (child2->typeDecl == intType) {
			second = 3;
		} else if (child2->typeDecl == ucharType) {
			second = 2;
		} else if (child2->typeDecl == charType) {
			second = 1;
		} else if (child2->typeDecl == nullType) {
			second = 0;
		} else {
			printf("Second argument doesn't correspond to any valid int type. \n");
		}
		if (second > first) {
			first = second;
		}
...
void ProcessExprCall(DAST* dast, char* startLabel, char* endLabel, int regBytes) {
  ...
  if (params->size > 0) {
    ...
    /* Decrement the stack. */
		size_t curoff, cursize;
    EmitADDI(SP, SP, -1 * decr);
    for (int i = 0; i < params->size; i++) {
      /* STUDENT CODE HERE */
			DAST* cur = params->children[i];
			Dispatch(cur, startLabel, endLabel, regBytes);
			curoff = *(offsets+i);
			//printf("curoff:%d\n",(int)curoff);
			cursize = *(sizes+i);
			//printf("cursize:%d\n\n",(int)cursize);
			if ((cursize < 0) || (cursize > 4)) {
				printf("Cursize not valid. \n");
				return;
			}
			if (cursize == 4) {
				EmitSW(S1,curoff,SP);
  ...
  /* STUDENT CODE HERE */
	//EmitADD(A0,RA,X0); //Put return address in A0
...
void ProcessIfElse(DAST* dast, char* startLabel, char* endLabel, int regBytes) {
  ...
  /* STUDENT CODE HERE. */
	Dispatch(condition,startLabel,endLabel,regBytes); // Find value of condition
	EmitBEQ(X0,S1,elseStart); // If condition equal to zero, we direct to condEnd
	Dispatch(ifBody,startLabel,endLabel,regBytes);
  EmitJ(condEnd); // Jumps into condEnd after ifbody executes
	EmitLABEL(elseStart);
	if (elseBody != NULL) {
...
void ProcessWhile(DAST* dast, char* startLabel, char* endLabel, int regBytes) {
  ...
  /* STUDENT CODE HERE. */
	...
	Dispatch(condition,startLabel,endLabel,regBytes); // Find value of condition
	EmitBEQ(X0,S1,endWhile); // If condition equal to zero, we direct to condEnd
